...
type inistate = rule list

(* type rule_group = inistate * rule list *)
...
let add_stt stt expr = match stt with
  ...
  | Stt_Is inistate -> { expr with initial_state = inistate }
  (* | Stt_Rg (is, rs) -> { expr with rule_groups= (is, rs) :: expr.rule_groups } *)
...
let string_of_inistate is = 
  match is with
      [] -> ""
    | _  -> "IS: [" ^ String.concat "" (List.map string_of_rule is) ^ "]\n"
